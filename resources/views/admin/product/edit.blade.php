...
<div class="content">
    ...
    <div class="container-extra">
        <form action="admin/product/edit/{{ $product->pk_product_id }}" method="post" enctype= "multipart/form-data">
            ...
            <table cellpadding="10" style="width: 80%; margin-bottom: 10px;">
                ...
                <tr>
                    <td>Tiêu đề: </td>
                    <td><input type="text" name="c_name" placeholder="Tên danh mục" value="{{$product->c_name}}" id="input"></td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="pk_category_id" id="pk_category_id">
                            ...
                            <option 
                            @if ($product->categoryDetail->fk_category_detail_id == $category->pk_category_id)
                                {{ "selected" }}
                            @endif
                            value="{{$category->pk_category_id}}">{{$category->c_name}}</option>
                            @endforeach              
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <select name="pk_category_detail_id" id="pk_category_detail_id">
                            ...
                            <option 
                            @if ($product->fk_product_id == $cd->pk_category_detail_id)
                                {{ "selected" }}
                            @endif
                            value="{{$cd->pk_category_detail_id}}">{{$cd->c_name}}</option>
                            @endforeach               
                        </select>
                    </td>
                </tr>
                ...
                <tr>
                    <td>Giới thiệu : </td>
                    <td style="width: 70%">
                        <textarea name="c_description" style="height:250px;">
                ...
                <tr>
                    <td>Giá: </td>
                    <td><input type="text" name="c_price" placeholder="Giá" value="{{$product->c_price}}" id="input"></td>
                </tr>

                <tr>
                    <td>Hot :</td>
                    <td><input type="checkbox" 
                        @if (isset($product->c_hotproduct) && $product->c_hotproduct==1)
                            {{ "checked" }}
                        @endif 
                        name="c_hotproduct" id="c_hotproduct"> </td>
                </tr>
